...
Create a proposal
## dfcio.msig::propose    proposer proposal_name requested trx
...
Approve a proposal
## dfcio.msig::approve    proposer proposal_name level
   - **proposer** account proposing a transaction
...
## dfcio.msig::unapprove    proposer proposal_name level
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
...
Cancel a proposal
## dfcio.msig::cancel    proposer proposal_name canceler
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
   - **canceler** account canceling the transaction (only proposer can cancel not expired transaction)

Execute a proposal
## dfcio.msig::exec    proposer proposal_name executer
   - **proposer** account proposing a transaction
   - **proposal_name** name of the proposal
...
Prerequisites:
   - dfcio.token contract installed to dfcio.token accountm, dfcio.msig contract installed on dfcio.msig account which is a priviliged account.
...
````
$ clfsc multisig propose test '[{"actor": "treasury", "permission": "active"}]' '[{"actor": "treasury", "permission": "active"}]' dfcio.token issue '{"to": "tester", "quantity": "1000.0000 FSC", "memo": ""}' -p tester
executed transaction: e26f3a3a7cba524a7b15a0b6c77c7daa73d3ba9bf84e83f9c2cdf27fcb183d61  336 bytes  107520 cycles
#    dfcio.msig <= dfcio.msig::propose          {"proposer":"tester","proposal_name":"test","requested":[{"actor":"treasury","permission":"active"}]...
````
...
#    dfcio.msig <= dfcio.msig::approve          {"proposer":"tester","proposal_name":"test","level":{"actor":"treasury","permission":"active"}}
````
...
#    dfcio.msig <= dfcio.msig::exec             {"proposer":"tester","proposal_name":"test","executer":"tester"}
````
