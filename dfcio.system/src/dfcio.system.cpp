...
namespace fsciosystem {
   ...
   void system_contract::setalimits( name account, int64_t ram, int64_t net, int64_t cpu ) {
      ...
      fscio_assert( ritr == userres.end(), "only supports unlimited accounts" );
      ...
      if( vitr != _voters.end() ) {
         ...
         fscio_assert( !(ram_managed || net_managed || cpu_managed), "cannot use setalimits on an account with managed resources" );
      }
   ...
   void system_contract::setacctram( name account, std::optional<int64_t> ram_bytes ) {
      ...
      if( !ram_bytes ) {
         auto vitr = _voters.find( account.value );
         fscio_assert( vitr != _voters.end() && has_field( vitr->flags1, voter_info::flags1_fields::ram_managed ),
         ...
         fscio_assert( *ram_bytes >= 0, "not allowed to set RAM limit to unlimited" );
   ...
   void system_contract::setacctnet( name account, std::optional<int64_t> net_weight ) {
      ...
      if( !net_weight ) {
         auto vitr = _voters.find( account.value );
         fscio_assert( vitr != _voters.end() && has_field( vitr->flags1, voter_info::flags1_fields::net_managed ),
         ...
         fscio_assert( *net_weight >= -1, "invalid value for net_weight" );
   ...
   void system_contract::setacctcpu( name account, std::optional<int64_t> cpu_weight ) {
      ...
      if( !cpu_weight ) {
         auto vitr = _voters.find( account.value );
         fscio_assert( vitr != _voters.end() && has_field( vitr->flags1, voter_info::flags1_fields::cpu_managed ),
         ...
         fscio_assert( *cpu_weight >= -1, "invalid value for cpu_weight" );
   ...
   void system_contract::updtrevision( uint8_t revision ) {
      require_auth( _self );
      fscio_assert( _gstate.revision < 255, "can not increment revision" ); // prevent wrap around
      fscio_assert( revision == _gstate.revision + 1, "can only increment revision by one" );
      fscio_assert( revision <= 1, // set upper bound to greatest revision supported in the code
   ...
   void system_contract::bidname( name bidder, name newname, asset bid ) {
      require_auth( bidder );
      fscio_assert( newname.suffix() == newname, "you can only bid on top-level suffix" );

      fscio_assert( (bool)newname, "the empty name is not a valid account name to bid on" );
      fscio_assert( (newname.value & 0xFull) == 0, "13 character names are not valid account names to bid on" );
      fscio_assert( (newname.value & 0x1F0ull) == 0, "accounts with 12 character names and no dots can be created without bidding required" );
      fscio_assert( !is_account( newname ), "account already exists" );
      fscio_assert( bid.symbol == core_symbol(), "asset must be system token" );
   ...
   void system_contract::bidrefund( name bidder, name newname ) {
      ...
      fscio_assert( it != refunds_table.end(), "refund not found" );
      INLINE_ACTION_SENDER(fscio::token, transfer)(
   ...
   void native::setabi( name acnt, const std::vector<char>& abi ) {
      fscio::multi_index< "abihash"_n, abi_hash >  table(_self, _self.value);
   ...
   void system_contract::init( unsigned_int version, symbol core ) {
      require_auth( _self );
      fscio_assert( version.value == 0, "unsupported version for init action" );

      auto itr = _rammarket.find(ramcore_symbol.raw());
      fscio_assert( itr == _rammarket.end(), "system contract has already been initialized" );

      auto system_token_supply   = fscio::token::get_supply(token_account, core.code() );
      fscio_assert( system_token_supply.symbol == core, "specified core symbol does not exist (precision mismatch)" );

      fscio_assert( system_token_supply.amount > 0, "system token supply must be greater than 0" );
...
DFCIO_DISPATCH( fsciosystem::system_contract,
     ...
     (rmvproducer)(updtrevision)(bidname)(bidrefund)(setresadcfg)
     // delegate_bandwidth.cpp
     (buyramkbytes)(sellram)(delegatebw)(undelegatebw)(refund)
     // voting.cpp
