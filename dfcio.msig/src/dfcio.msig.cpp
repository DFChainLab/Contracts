#include <fscio.msig/fscio.msig.hpp>
#include <fsciolib/action.hpp>
#include <fsciolib/permission.hpp>
#include <fsciolib/crypto.hpp>
                        ...
                        ignore<transaction> trx )
{
   ...
   fscio_assert( _trx_header.expiration >= fscio::time_point_sec(current_time_point()), "transaction expired" );
   //fscio_assert( trx_header.actions.size() > 0, "transaction must have at least one action" );

   proposals proptable( _self, _proposer.value );
   fscio_assert( proptable.find( _proposal_name.value ) == proptable.end(), "proposal with the same name exists" );
   ...
   fscio_assert( res > 0, "transaction authorization failed" );
                        ...
                        const fscio::binary_extension<fscio::checksum256>& proposal_hash )
{
   ...
   if ( apps_it != apptable.end() ) {
      ...
      fscio_assert( itr != apps_it->requested_approvals.end(), "approval is not on the list of requested approvals" );
      ...
      fscio_assert( itr != apps.requested_approvals.end(), "approval is not on the list of requested approvals" );
...
void multisig::unapprove( name proposer, name proposal_name, permission_level level ) {
   ...
   if ( apps_it != apptable.end() ) {
      ...
      fscio_assert( itr != apps_it->provided_approvals.end(), "no approval previously granted" );
      apptable.modify( apps_it, proposer, [&]( auto& a ) {
      ...
      fscio_assert( itr != apps.provided_approvals.end(), "no approval previously granted" );
      old_apptable.modify( apps, proposer, [&]( auto& a ) {
...
void multisig::cancel( name proposer, name proposal_name, name canceler ) {
   ...
   if( canceler != proposer ) {
      fscio_assert( unpack<transaction_header>( prop.packed_transaction ).expiration < fscio::time_point_sec(current_time_point()), "cannot cancel until expiration" );
   }
   ...
   if ( apps_it != apptable.end() ) {
      ...
      fscio_assert( apps_it != old_apptable.end(), "proposal not found" );
...
void multisig::exec( name proposer, name proposal_name, name executer ) {
   ...
   ds >> trx_header;
   fscio_assert( trx_header.expiration >= fscio::time_point_sec(current_time_point()), "transaction expired" );
...
DFCIO_DISPATCH( fscio::multisig, (propose)(approve)(unapprove)(cancel)(exec)(invalidate) )
