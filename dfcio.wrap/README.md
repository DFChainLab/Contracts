...
Execute a transaction while bypassing regular authorization checks (requires authorization of fscio.wrap which needs to be a privileged account).

### fscio.wrap::exec    executer trx
...
## 2. Installing the fscio.wrap contract

The fscio.wrap contract needs to be installed on a privileged account to function. It is recommended to use the account `fscio.wrap`.

First, the account `fscio.wrap` needs to be created. Since it has the restricted `fscio.` prefix, only a privileged account can create this account. So this guide will use the `fscio` account to create the `fscio.wrap` account. On typical live blockchain configurations, the `fscio` account can only be controlled by a supermajority of the current active block producers. So, this guide will use the `fscio.msig` contract to help coordinate the approvals of the proposed transaction that creates the `fscio.wrap` account.

The `fscio.wrap` account also needs to have sufficient RAM to host the contract and sufficient CPU and network bandwidth to deploy the contract. This means that the creator of the account (`fscio`) needs to gift sufficient RAM to the new account and delegate (preferably with transfer) sufficient bandwidth to the new account. To pull this off the `fscio` account needs to have enough of the core system token (the `SYS` token will be used within this guide) in its liquid balance. So prior to continuing with the next steps of this guide, the active block producers of the chain who are coordinating this process need to ensure that a sufficient amount of core system tokens that they are authorized to spend is placed in the liquid balance of the `fscio` account.

This guide will be using clfsc to carry out the process.

### 2.1 Create the fscio.wrap account

#### 2.1.1 Generate the transaction to create the fscio.wrap account

The transaction to create the `fscio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clfsc command to propose the transaction to the fscio.msig contract.

A simple way to generate a transaction to create a new account is to use the `clfsc system newaccount`. However, that sub-command currently only accepts a single public key as the owner and active authority of the new account. However, the owner and active authorities of the new account should only be satisfied by the `active` permission of `fscio`. One option is to create the new account with the some newly generated key, and then later update the authorities of the new account using `clfsc set account permission`. This guide will take an alternative approach which atomically creates the new account in its proper configuration.

Three unsigned transactions will be generated using clfsc and then the actions within those transactions will be appropriately stitched together into a single transaction which will later be proposed using the fscio.msig contract.

First, generate a transaction to capture the necessary actions involved in creating a new account:
```
$ clfsc system newaccount -s -j -d --transfer --stake-net "1.000 SYS" --stake-cpu "1.000 SYS" --buy-ram-kbytes 50 fscio fscio.wrap FSC8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
726964ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea305500c80000"} arg: {"code":"fscio","action":"buyrambytes","args":{"payer":"fscio","receiver":"fscio.wrap","bytes":51200}}
726967ms thread-0   main.cpp:429                  create_action        ] result: {"binargs":"0000000000ea305500004d1a03ea3055102700000000000004535953000000001027000000000000045359530000000001"} arg: {"code":"fscio","action":"delegatebw","args":{"from":"fscio","receiver":"fscio.wrap","stake_net_quantity":"1.0000 SYS","stake_cpu_quantity":"1.0000 SYS","transfer":true}}
...
The actual public key used is not important since that data is only encoded into the `fscio::newaccount` action which will be replaced soon anyway.

Second, create a file (e.g. newaccount_payload.json) with the JSON payload for the real `fscio::newaccount` action. It should look like:
```
$ cat newaccount_payload.json
{
   ...
   "owner": {
      ...
      "keys": [],
      "accounts": [{
         "permission": {"actor": "fscio", "permission": "active"},
         "weight": 1
      }],
      "waits": []
   },
...
Third, generate a transaction containing the actual `fscio::newaccount` action that will be used in the final transaction:
```
$ clfsc push action -s -j -d fscio newaccount newaccount_payload.json -p fscio > generated_newaccount_trx.json
...
Fourth, generate a transaction containing the `fscio::setpriv` action which will make the `fscio.wrap` account privileged:
```
$ clfsc push action -s -j -d fscio setpriv '{"account": "fscio.wrap", "is_priv": 1}' -p fscio > generated_setpriv_trx.json
...
Next, the action JSONs of the previously generated transactions will be used to construct a unified transaction which will eventually be proposed with the fscio.msig contract. A good way to get started is to make a copy of the generated_newaccount_trx.json file (call the copied file create_wrap_account_trx.json) and edit the first three fields so it looks something like the following:
```
...
It will be useful to have a JSON of the active permissions of each of the active block producers for later when proposing transactions using the fscio.msig contract.

This guide will assume that there are 15 active block producers on the chain with account names: `blkproducera`, `blkproducerb`, ..., `blkproduceru`.
...
#### 2.1.2 Propose the transaction to create the fscio.wrap account
...
```
$ clfsc multisig propose_trx createwrap producer_permissions.json create_wrap_account_trx.json blkproducera
executed transaction: bf6aaa06b40e2a35491525cb11431efd2b5ac94e4a7a9c693c5bf0cfed942393  744 bytes  772 us
#    fscio.msig <= fscio.msig::propose          {"proposer":"blkproducera","proposal_name":"createwrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.3 Review and approve the transaction to create the fscio.wrap account
...
The proposed transaction can be reviewed using the `clfsc multisig review` command:
```
$ clfsc multisig review blkproducera createwrap > create_wrap_account_trx_to_review.json
...
```
$ clfsc multisig propose_trx -j -s -d createwrap '[]' create_wrap_account_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_create_wrap_trx_serialized.hex
...
```
$ clfsc multisig approve blkproducera createwrap '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 03a907e2a3192aac0cd040c73db8273c9da7696dc7960de22b1a479ae5ee9f23  128 bytes  472 us
#    fscio.msig <= fscio.msig::approve          {"proposer":"blkproducera","proposal_name":"createwrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.1.4 Execute the transaction to create the fscio.wrap account

When the necessary approvals are collected (in this example, with 15 block producers, at least 15 of their approvals were required), anyone can push the `fscio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the fscio.msig contract).

```
$ clfsc multisig exec blkproducera createwrap blkproducera
executed transaction: 7ecc183b99915cc411f96dde7c35c3fe0df6e732507f272af3a039b706482e5a  160 bytes  850 us
#    fscio.msig <= fscio.msig::exec             {"proposer":"blkproducera","proposal_name":"createwrap","executer":"blkproducera"}
warning: transaction executed locally, but may not be confirmed by the network yet
```

Anyone can now verify that the `fscio.wrap` was created:
```
$ clfsc get account fscio.wrap
privileged: true
permissions:
     owner     1:    1 fscio@active,
        active     1:    1 fscio@active,
memory:
...
### 2.2 Deploy the fscio.wrap contract

#### 2.2.1  Generate the transaction to deploy the fscio.wrap contract

The transaction to deploy the contract to the `fscio.wrap` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the clfsc command to propose the transaction to the fscio.msig contract.

The easy way to generate this transaction is using clfsc:
```
$ clfsc set contract -s -j -d fscio.wrap contracts/fscio.wrap/ > deploy_wrap_contract_trx.json
Reading WAST/WASM from contracts/fscio.wrap/fscio.wrap.wasm...
...
```
$ clfsc multisig propose_trx deploywrap producer_permissions.json deploy_wrap_contract_trx.json blkproducera
executed transaction: 9e50dd40eba25583a657ee8114986a921d413b917002c8fb2d02e2d670f720a8  4312 bytes  871 us
#    fscio.msig <= fscio.msig::propose          {"proposer":"blkproducera","proposal_name":"deploywrap","requested":[{"actor":"blkproducera","permis...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.3 Review and approve the transaction to deploy the fscio.wrap contract

Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.

The proposed transaction can be reviewed using the `clfsc multisig review` command:
```
$ clfsc multisig review blkproducera deploywrap > deploy_wrap_contract_trx_to_review.json
...
```
$ clfsc multisig propose_trx -j -s -d deploywrap '[]' deploy_wrap_contract_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_deploy_wrap_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clfsc multisig approve blkproducera deploywrap '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: d1e424e05ee4d96eb079fcd5190dd0bf35eca8c27dd7231b59df8e464881abfd  128 bytes  483 us
#    fscio.msig <= fscio.msig::approve          {"proposer":"blkproducera","proposal_name":"deploywrap","level":{"actor":"blkproducerb","permission"...
warning: transaction executed locally, but may not be confirmed by the network yet
```

#### 2.2.4 Execute the transaction to create the fscio.wrap account

When the necessary approvals are collected (in this example, with 15 block producers, at least 15 of their approvals were required), anyone can push the `fscio.msig::exec` action which executes the approved transaction. It makes a lot of sense for the lead block producer who proposed the transaction to also execute it (this will incur another temporary RAM cost for the deferred transaction that is generated by the fscio.msig contract).

```
$ clfsc multisig exec blkproducera deploywrap blkproducera
executed transaction: e8da14c6f1fdc3255b5413adccfd0d89b18f832a4cc18c4324ea2beec6abd483  160 bytes  1877 us
#    fscio.msig <= fscio.msig::exec             {"proposer":"blkproducera","proposal_name":"deploywrap","executer":"blkproducera"}
```

Anyone can now verify that the `fscio.wrap` contract was deployed correctly.

```
$ clfsc get code -a retrieved-fscio.wrap.abi fscio.wrap
code hash: 1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c
saving abi to retrieved-fscio.wrap.abi
$ sha256sum contracts/fscio.wrap/fscio.wrap.wasm
1b3456a5eca28bcaca7a2a3360fbb2a72b9772a416c8e11a303bcb26bfe3263c  contracts/fscio.wrap/fscio.wrap.wasm
```

If the two hashes match then the local WebAssembly code is the one deployed on the blockchain. The retrieved ABI, which was stored in the file retrieved-fscio.wrap.abi, can then be compared to the original ABI of the contract (contracts/fscio.wrap/fscio.wrap.abi) to ensure they are semantically the same.

## 3. Using the fscio.wrap contract

### 3.1 Example: Updating owner authority of an arbitrary account

This example will demonstrate how to use the deployed fscio.wrap contract together with the fscio.msig contract to allow a greater than two-thirds supermajority of block producers of an DFCIO blockchain to change the owner authority of an arbitrary account. The example will use clfsc: in particular, the `clfsc multisig` command, the `clfsc set account permission` sub-command, and the `clfsc wrap exec` sub-command. However, the guide also demonstrates what to do if the `clfsc wrap exec` sub-command is not available.

This guide assumes that there are 15 active block producers on the chain with account names: `blkproducera`, `blkproducerb`, ..., `blkproduceru`. Block producer `blkproducera` will act as the lead block producer handling the proposal of the transaction.
...
```
permissions:
     owner     1:    1 FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
        active     1:    1 FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV
memory:
...
Assume that none of the block producers know the private key corresponding to the public key `FSC6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV` which, as can be seen above, is initially securing access to the `alice` account.
...
```
$ clfsc set account permission -s -j -d alice owner '{"threshold": 1, "accounts": [{"permission": {"actor": "fscio", "permission": "active"}, "weight": 1}]}' > update_alice_owner_trx.json
```
...
The next step is to generate the transaction containing the `fscio.wrap::exec` action. This action will contain the transaction in update_alice_owner_trx.json as part of its action payload data.

```
$ clfsc wrap exec -s -j -d blkproducera update_alice_owner_trx.json > wrap_update_alice_owner_trx.json
```
...
If the `clfsc wrap` command is not available, there is an alternative way to generate the above transaction. There is no need to continue reading the remaining of sub-section 3.1.1 if the wrap_update_alice_owner_trx.json file was already generated with content similar to the above using the `clfsc wrap exec` sub-command method.
...
```
$ clfsc multisig propose_trx -s -j -d nothing '[]' update_alice_owner_trx.json nothing | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > update_alice_owner_trx_serialized.hex
...
Then generate the template for the transaction containing the `fscio.wrap::exec` action:
```
$ clfsc push action -s -j -d fscio.wrap exec '{"executer": "blkproducera", "trx": ""}' > wrap_update_alice_owner_trx.json
$ cat wrap_update_alice_owner_trx.json
{
...
```
$ clfsc multisig propose_trx updatealice producer_permissions.json wrap_update_alice_owner_trx.json blkproducera
executed transaction: 10474f52c9e3fc8e729469a577cd2fc9e4330e25b3fd402fc738ddde26605c13  624 bytes  782 us
#    fscio.msig <= fscio.msig::propose          {"proposer":"blkproducera","proposal_name":"updatealice","requested":[{"actor":"blkproducera","permi...
warning: transaction executed locally, but may not be confirmed by the network yet
```
...
Each of the potential approvers of the proposed transaction (i.e. the active block producers) should first review the proposed transaction to make sure they are not approving anything that they do not agree to.
```
$ clfsc multisig review blkproducera updatealice > wrap_update_alice_owner_trx_to_review.json
...
The approvers should go through the human-readable transaction output and make sure everything looks fine. However, due to a current limitation of nodfsc/clfsc, the JSONification of action payload data does not occur recursively. So while both the `hex_data` and the human-readable JSON `data` of the payload of the `fscio.wrap::exec` action is available in the output of the `clfsc multisig review` command, only the hex data is available of the payload of the inner `fscio::updateauth` action. So it is not clear what the `updateauth` will actually do.

Furthermore, even if this usability issue was fixed in nodfsc/clfsc, there will still be cases where there is no sensible human-readable version of an action data payload within a transaction. An example of this is the proposed transaction in sub-section 2.2.3 which used the `fscio::setcode` action to set the contract code of the `fscio.wrap` account. The best thing to do in such situations is for the reviewer to compare the proposed transaction to one generated by them through a process they trust.
...
```
$ clfsc multisig propose_trx -j -s -d updatealice '[]' wrap_update_alice_owner_trx.json blkproducera | grep '"data":' | sed 's/^[ \t]*"data":[ \t]*//;s/[",]//g' | cut -c 35- > expected_wrap_update_alice_owner_trx_serialized.hex
...
When an approver (e.g. `blkproducerb`) is satisfied with the proposed transaction, they can simply approve it:
```
$ clfsc multisig approve blkproducera updatealice '{"actor": "blkproducerb", "permission": "active"}' -p blkproducerb
executed transaction: 2bddc7747e0660ba26babf95035225895b134bfb2ede32ba0a2bb6091c7dab56  128 bytes  543 us
#    fscio.msig <= fscio.msig::approve          {"proposer":"blkproducera","proposal_name":"updatealice","level":{"actor":"blkproducerb","permission...
warning: transaction executed locally, but may not be confirmed by the network yet
```
