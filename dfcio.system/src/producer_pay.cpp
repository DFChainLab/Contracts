#include <dfcio.system/dfcio.system.hpp>

#include <dfcio.token/dfcio.token.hpp>
#include <math.h>
...
namespace dfciosystem {
   ...
   void system_contract::claimprod( const name owner){
      ...
      dfcio_assert( (ct - prod.last_claim_time).count() >= claim_prod_rewards_preiod, "already claimed rewards within past day" );
      ...
      if( prod.rewards_producer_block_pay_balance > 0 ) {
         INLINE_ACTION_SENDER(dfcio::token, transfer)(
   ...
   void system_contract::claimvoter( const name owner, const name producer ) {
      ...
      dfcio_assert( ( ct - voter.last_claim_time ).count() >= claim_voter_rewards_preiod, "already claimed rewards within past preiod" );
      ...
      dfcio_assert( newest_total_voteage > 0, "claim is not available yet" );
      ...
      dfcio_assert( 0 <= vote_reward && vote_reward <= prod.rewards_voters_vote_pay_balance, "vote_reward don't count" );
      dfcio_assert( 0 <= block_reward && block_reward <= prod.rewards_voters_block_pay_balance, "block_reward don't count" );
   ...
   void system_contract::distribute_voters_rewards( const time_point distribut_time, const name producer ) {
      ...
      if ( usecs_since_last_fill > 0 && _gstate.last_pervote_bucket_fill > time_point() ) {
         const asset token_supply = dfcio::token::get_supply(token_account, core_symbol().code());
         ...
         INLINE_ACTION_SENDER(dfcio::token, issue)(
   ...
   void system_contract::require_activated() {
      dfcio_assert( _gstate.total_activated_stake >= get_min_activated_stake(), "cannot claim rewards until the chain is activated" );
   }
} //namespace dfciosystem
